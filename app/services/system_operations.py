...
def backup_database_if_due(now: float | None = None) -> str | None:
    ...
    target_dir = os.path.abspath(settings.BACKUP_PATH)
    ...
    if backups and timestamp - os.path.getmtime(backups[-1]) < settings.BACKUP_INTERVAL_DAYS * 86400:
        return None
    ...
    try:
        ...
        for old in backups[:max(0, len(backups) + 1 - settings.BACKUP_RETENTION)]:
            os.remove(old)
        return target
...
def send_alert_notification(total: int) -> bool:
    if not settings.NOTIFICATION_WEBHOOK or total <= 0:
        return False
    payload = json.dumps({"event": "sentiment_alerts", "count": int(total)}).encode("utf-8")
    try:
        response = safe_http_request(
            settings.NOTIFICATION_WEBHOOK, method="POST",
