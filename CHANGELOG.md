...
- 修复 #121：天气 script 工具同步 `message` 参数契约，外部代理兼容 `{param}` 与 `{{param}}` URL 模板并统一编码。
- 修复 #155：脚本型采集工具种子 SQL 列值映射、数组 Schema 和重复初始化保持正确。
- 合并后补齐 23 个本地接口的注册表测试，完整回归通过。
...
- 合并中文路径 Haar cascade 回退，并捕获 LBPH 原生工厂初始化异常。
- 删除人脸数据时同步清理陈旧模型；组件恢复后可从剩余图片自动重建。
- 统一接口架构的采集入口恢复未解析 URL 占位符校验，并更新相关测试契约。
...
- 新增可执行的数据库备份、Webhook 舆情通知、系统配置范围校验及旧数据库增量配置迁移。
