...
管理侧数智大屏 (`/admin/dashboard`) 提供自适应比例的运营驾驶舱：3D 地球按真实采集来源聚合点位并标注来源名称、数量、今日新增、深采数量和域名；词云从标题/摘要/采集关键词提取，趋势图、来源分布、深采占比、来源 Top 列表和最新入库动态同步刷新。未知或自定义数据源也会通过稳定地理兜底映射显示在地球上，不再依赖前端随机模拟点；排行榜模板使用 Tornado 原生循环索引渲染，避免 `loop.index` 兼容问题导致页面 500。
