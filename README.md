...
- **手势识别说明**：聊天页顶部“手势”旁提供“说明”入口，摄像头预览与操作说明分区展示，避免视频或骨架画布遮挡 `✌️ 剪刀手 → @天气 [当前城市]`、`✊ 握拳 → @随机音乐`、`✋ 手掌 → @新闻聚合` 的映射和冷却规则。剪刀手会读取浏览器系统定位并经后端反查城市，实际发送 `@天气 城市名`；定位失败或用户拒绝定位时使用系统默认城市“成都”。识别成功后自动关闭摄像头并返回对话界面，流式输出中触发的识别会被忽略且不关闭摄像头。
