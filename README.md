...
> **v1.2.0 新增**：管理侧数智大屏（Three.js 数字地球 + Canvas 兜底 + 词云 + 数据可视化、ECharts/Wordcloud）、舆情大屏（敏感词预警 + AI 风析、实时扫描）。
...
✋ 手势交互 — 摄像头实时手势识别        — 剪刀手定位当前城市查天气，握拳/手掌触发音乐/新闻，识别成功自动关闭
...
│   │   ├── admin_skill.py        # 技能管理 CRUD + MCP 工具绑定（v0.5）
│   │   ├── admin_mcp.py          # MCP 工具管理 CRUD + 测试 + 热重载（v0.10）
│   │   └── admin_interface.py    # 接口管理 CRUD + 测试（v0.9）
│   │
...
│   │   ├── skill.py              # SkillRepository — 技能仓储（v0.5）
│   │   ├── mcp_tool.py           # MCPToolRepository — MCP 工具仓储（v0.10）
│   │   └── api_interface.py      # ApiInterfaceRepository — 接口模板仓储（v0.9）
│   │
│   ├── mcp/                      # MCP 协议模块（v0.4）
│   │   ├── __init__.py           # 模块入口
...
│   │       ├── skill_list.html        # 技能列表（v0.5）
│   │       ├── skill_form.html        # 技能表单（v0.5）
│   │       ├── mcp_tool_list.html     # MCP 工具列表（v0.10）
│   │       ├── mcp_tool_form.html     # MCP 工具表单（v0.10）
...
```
==================================================
  瞭望与问数系统 (DataFinderAgentOS) v{{ app_version }}
...
| v0.5 | 添加 `data_warehouse_fts` FTS5 虚拟表 + 同步触发器 |
| v0.6 | 添加 `watch_sources.schedule_interval` 列 |
| v0.9 | 添加 `api_interfaces` 表与 `digital_employees.api_interface_id`（接口管理联动） |
| v0.10 | 添加 `mcp_tools` / `mcp_tool_test_logs` 表（MCP 工具数据库驱动注册中心） |
| v0.10 | 添加 `skills.mcp_tool_id` 列 + `digital_employees.mcp_tool_ids` 列 |
| v0.10 | 种子数据迁移：18 个内置 MCP 工具 + 旧 TAG → Skill ID 迁移 |
...
| 新增用户 | 用户名 + 密码 + 角色选择；密码至少8位并包含至少两类字符 |
| 编辑用户 | 修改密码（留空不修改）、更换角色；新密码执行相同强度校验 |
| 启用/禁用 | 单个切换 + 批量切换 |
| 删除用户 | 单个删除 + 批量删除 |
...
数据仓库 (`/admin/warehouse`) 是瞭望采集结果的**独立存储与检索中心**。v0.2 起采用独立的 `data_warehouse` 表（区别于原始采集结果表 `watch_results`），支持 URL 去重和深度采集扩展。
...
管理侧数智大屏 (`/admin/dashboard`) 提供自适应比例的运营驾驶舱：3D 地球作为大屏模块内嵌展示，整体采用“主地球 + 右侧信息宫格 + 底部等高图表”的协调排布。地球优先使用 Three.js/WebGL 和真实地球纹理渲染数字地球（星空、云层、大气辉光、飞线、点位发光和标签），用户无需安装任何浏览器插件；若 CDN 或 WebGL 不可用，会自动回退到系统内置原生 Canvas 地球，避免模块空白。地球按真实采集来源聚合点位，标注来源名称、数量、今日新增、深采数量和有效域名；`example`/`example.*` 这类演示域名会自动隐藏，避免污染运营展示。词云从标题/摘要/采集关键词提取，趋势图、来源分布、深采占比、来源 Top 列表和最新入库动态同步刷新。未知或自定义数据源也会通过稳定地理兜底映射显示在地球上，不再依赖前端随机模拟点；排行榜模板使用 Tornado 原生循环索引渲染，避免 `loop.index` 兼容问题导致页面 500。
...
| **B区** | 模型切换器 | 下拉选择启用的 AI 模型，⭐标识默认模型；有权限用户显示“配置模型 API”入口 |
...
- **手势识别说明**：聊天页顶部“手势”旁提供“说明”入口，摄像头预览与操作说明分区展示，避免视频或骨架画布遮挡 `✌️ 剪刀手 → @天气 [当前城市]`、`✊ 握拳 → @随机音乐`、`✋ 手掌 → @新闻聚合` 的映射和冷却规则。摄像头启动后会后台预解析当前城市，剪刀手实际发送 `@天气 城市名`，从而降低首次触发等待；定位失败时内部使用系统默认城市兜底但不在手势说明区展示实现细节。识别成功后自动关闭摄像头并返回对话界面，流式输出中触发的识别会被忽略且不关闭摄像头。
- **人脸登录确认**：账户页人脸注册采用“拍照预览 → 确认使用”流程；“启用人脸识别登录”是独立开关，未录入人脸也可先保存启用状态，真正登录时若尚未录入会提示先录入人脸信息；人脸登录接口不做失败频率锁定，便于摄像头/识别反复重试；人脸文件使用 Unicode 安全读写，避免中文项目路径下录入假成功；识别侧使用单模板最大脸检测、带边距裁剪和 CLAHE 光照归一化提升稳定性
...
- 归属校验：仅对话创建者可查看/删除自己的对话；删除失败会在前端明确提示，删除后的会话不会被迟到的流式保存重新复活
- 管理侧会话管理：管理员可在 `/admin/conversation` 跨用户查看会话列表、按用户筛选、查看消息详情并强制删除异常会话

#### 8.5 语音合成播报（TTS）

v0.9 新增的 **Edge TTS 语音合成播报**功能，为每条 AI 回复消息提供一键朗读能力。
...
| 类型 | 实现方式 | 适用场景 |
|------|----------|----------|
| **LLM 型** | 绑定 AI 模型 + 系统提示词 + 技能列表 + MCP 工具权限（含 Crawl4ai 深度采集） | 复杂推理、数据分析、文案撰写、深度采集 |
...
| 🎵 **随机音乐** | API (MCP) | 通过 get_random_music 工具调用 Meting API，从网易云热歌榜随机推荐歌曲并展示音乐卡片 |
...
- **MCP 工具权限控制（v0.8）**：员工编辑页可勾选允许使用的 MCP 工具，仅勾选且已启用的工具才对员工可用（最小权限原则）。未配置任何工具时，员工无权调用任何 MCP 工具。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/chat` | 用户前台对话主页（A/B/C/D/E 五区，有权限时含模型 API 配置入口） |
...
erDiagram
    ...
    digital_employees {
        ...
        string system_prompt
        string skills
        int crawl4ai_enabled      // [v0.8 废弃] 改用 mcp_tool_ids
        string mcp_tool_ids        // [v0.8] MCP 工具权限 JSON 数组 "[1,2,3]"
        string api_url
        string api_method
        string api_headers
        string api_params_template
...
| `mcp_tools` | MCP 工具注册表（v0.10） | 18 ~ 100 | `name`(UNIQUE), `category` |
| `mcp_tool_test_logs` | MCP 工具测试日志（v0.10） | 自动增长 | `tool_id`(CASCADE) |
| `skills` | 技能库表（v0.5） | 5 ~ 100 | `name`(UNIQUE), `mcp_tool_id` |
...
```
Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: https: blob:; media-src 'self' blob: https:; font-src 'self' https://cdn.jsdelivr.net; connect-src 'self' https://cdn.jsdelivr.net blob: https://*.jsdelivr.net; worker-src 'self' blob:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'
...
| 版本 | 变更内容 |
|------|---------|
| v0.2 | 添加 `ai_models.total_tokens` 列（Token 消耗累加统计） |
| v0.2 | 添加 `audit_logs` 表（操作审计日志，含 3 个索引） |
| v0.2 | 添加安全相关索引（`audit_logs.action`、`audit_logs.username`、`audit_logs.created_at`） |
| v0.2 | 添加 `data_warehouse` 独立表（标题/链接/摘要/来源 + URL 去重索引） |
| v0.5 | 添加 `conversations` / `conversation_messages` 表（多轮对话持久化） |
| v0.4 | 添加 `digital_employees` 表（数字化员工配置） |
| v0.5 | 添加 `data_warehouse_fts` 虚拟表 + 3 个同步触发器（FTS5 全文检索） |
| v0.9 | 添加 `api_interfaces` 表与 `digital_employees.api_interface_id`（接口管理联动） |
| v0.10 | 添加 `mcp_tools` / `mcp_tool_test_logs` 表 + `skills.mcp_tool_id` / `digital_employees.mcp_tool_ids` 列 |
| v0.10 | 种子数据迁移：18 个内置 MCP 工具 + 旧 TAG → Skill ID 数据迁移 |
