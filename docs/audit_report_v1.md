...
> 以下记录各漏洞在后续版本中的实际修复情况。版本号对应 `CHANGELOG.md`。

### 5.1 已修复漏洞

| 审计编号 | 严重度 | 修复版本 | 修复 Commit | 修复说明 |
|---------|--------|---------|------------|---------|
| CRITICAL-001 | 🔴 严重 | v1.0.2-beta | `f502a52` | 默认管理员密码改为随机生成，不再写入进程日志（#55） |
| CRITICAL-001 | 🔴 严重 | v1.0.3-beta | `263b15d` | SEC-006：首次启动强制修改弱口令 |
| CRITICAL-001 | 🔴 严重 | v1.3.4-beta | `53406c6` | #125：密码策略强化至 8 字符 + 2 类字符复杂度 |
| CRITICAL-003 | 🔴 严重 | v0.x | `4cbc4d0` | #11：API 员工 URL 校验 SSRF TOCTOU DNS 重绑定修复 |
| CRITICAL-003 | 🔴 严重 | v1.0.0-beta | — | 服务端外呼统一 DNS 固定（`pin_url_to_ip()`）、禁重定向、响应上限 |
| CRITICAL-003 | 🔴 严重 | v1.0.3-beta | `263b15d` | SEC-003：多条服务端外呼链路完整 IP 固定与重定向校验 |
| CRITICAL-003 | 🔴 严重 | v1.3.4-beta | `53406c6` | #122：`media_generator.py` 统一使用 `safe_http_request` 防 SSRF |
| CRITICAL-003 | 🔴 严重 | v1.7.0-beta | `48102de` | MCP 统一调度架构内置 SSRF 防护 |
| HIGH-001 | 🟠 高危 | v1.0.2-beta | `f502a52` | #2：Mock 回复移除 system prompt 泄露，仅显示配置状态 |
| HIGH-002 | 🟠 高危 | v1.0.3-beta | `263b15d` | AI 模型读取不再默认解密 API Key，改为按需解密模式 |
| HIGH-004 | 🟠 高危 | v1.0.3-beta | `263b15d` | SEC-003：管理端 API 员工 DNS 固定（与用户端对齐） |
| HIGH-004 | 🟠 高危 | v1.7.0-beta | `48102de` | API 型员工 MCP 统一调度，安全 HTTP 调用统一 |
| HIGH-005 | 🟠 高危 | v1.3.4-beta | `53406c6` | #80：`media_generator.py` 第三方 API 错误消息不再泄露内部细节 |
| HIGH-005 | 🟠 高危 | v1.3.4-beta | `53406c6` | #58：采集器/深度采集器 Brotli 回退异常处理改进 |
| HIGH-006 | 🟠 高危 | v1.0.3-beta | `263b15d` | CRLF 检测应用于自定义 HTTP 头部键值 |
| HIGH-007 | 🟠 高危 | v1.0.2-beta | `f502a52` | #1：数据仓库内容拼入 system prompt 前脱敏（XML 包裹 + 高危模式过滤 + 长度截断） |
| MED-007 | 🟡 中危 | v1.0.3-beta | `263b15d` | `collector.py` / `deep_collector.py` 重定向处理逻辑统一 |
| LOW-003 | 🔵 低危 | v1.3.4-beta | `53406c6` | #58：Brotli 解压失败不再回退为原始二进制 |

### 5.2 已缓解（有防护措施但仍存残余风险）

| 审计编号 | 严重度 | 状态 | 说明 |
|---------|--------|------|------|
| CRITICAL-002 | 🔴 严重 | ⚠️ 已缓解 | v0.x `aa44e57` #10：`.secret_key` 600 权限；v0.x `c9b8474` #8：`_ensure_secret_key()` 统一入口。Fernet 加密 + 文件权限保护（SF-06/SF-12）；KMS 建议未实施 |
| HIGH-003 | 🟠 高危 | ⚠️ 已缓解 | v1.1.0-beta `aa027bc`：消除 `self.write()` 异常消息 XSS；v1.3.4-beta `53406c6` #128：`write_error` 检查 DEBUG 模式。部分老代码可能仍使用 `self.write()` |
| HIGH-008 | 🟠 高危 | ⚠️ 残余风险 | CSP 已移除 `unsafe-eval`，添加 `frame-ancestors/base-uri/form-action`；`unsafe-inline` 因 Layui 遗留模板保留（已文档化） |
| MED-005 | 🟡 中危 | ⚠️ 已缓解 | v1.3.4-beta #111：TTS 异步文件读取改进；缓存 TTL 和用户级速率限制未实施 |
| MED-006 | 🟡 中危 | ⚠️ 已缓解 | 审计日志系统完善（v1.1.0-beta 起），日志访问控制加强 |
| LOW-004 | 🔵 低危 | ⚠️ 已接受 | 内存明文密钥为架构固有局限，加密前短暂存在 |

### 5.3 待修复

| 审计编号 | 严重度 | 状态 | 说明 |
|---------|--------|------|------|
| MED-001 | 🟡 中危 | 📋 待修复 | Cookie `secure` 标志仍依赖 `self.request.protocol`，未检查 `X-Forwarded-Proto` |
| MED-002 | 🟡 中危 | 📋 待修复 | `SECURITY_HEADERS` 仍缺少 `Strict-Transport-Security`（HSTS） |
| MED-003 | 🟡 中危 | 📋 待修复 | `X-XSS-Protection: 1; mode=block` 头仍存在（已被现代浏览器弃用） |
| MED-004 | 🟡 中危 | 📋 待修复 | 用户对话创建时未验证 `model_id` 是否有效/已启用 |
| LOW-001 | 🔵 低危 | 📋 低优先级 | 默认端口 `10010`，v1.3.4-beta 已加入 PORT 配置映射（#83） |
| LOW-002 | 🔵 低危 | 📋 低优先级 | `Permissions-Policy` 未限制 `clipboard-read/write`、`display-capture` |

### 5.4 后续审计轮次新增并已修复

> v1 审计后，项目通过多轮安全审查发现并修复了额外漏洞，此处列出与 v1 22 项不重叠的关键安全修复。

| 批次 | 版本 | Commit | 修复内容 |
|------|------|--------|---------|
| 第四轮 (#57-#76) | v1.0.3-beta | `263b15d` | 20 项：RBAC 细粒度越权修复、AI Markdown DOMPurify 净化、响应大小上限、登录限速器同步保护、禁用用户会话即时失效等 |
| 批量修复 (#77-#126) | v1.3.4-beta | `53406c6` | 10 项安全加固：#128 堆栈泄露、#126 SQL 注入（表名白名单）、#125 密码策略、#123 cryptography CVE、#122 SSRF、#119 信息泄露、#86 XSS、#81 白名单校验、#80 错误泄露、#79 隐私保护 |
| XSS 专项 | v0.x~v1.x | 多个 | #3 数据仓库链接清洗、#4 第三方 API 响应 HTML 转义、#5 `{% raw %}` 绕过修复、#6 HTML 实体编码 onclick XSS、#9 DOM-based XSS URL 参数校验 |
| 人脸识别安全 | v1.8.1-beta | `2c04fbe` | 人脸登录漏洞修复（前后端双重验证） |
