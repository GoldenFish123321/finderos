...
- 用户侧手势识别需提供基本说明入口，明确展示 `剪刀手→@天气 [当前城市]`、`握拳→@随机音乐`、`手掌→@新闻聚合` 的触发关系、摄像头/定位权限要求和冷却规则；剪刀手查询天气时需使用浏览器系统定位，经后端反查城市后发送 `@天气 城市名`，定位失败或用户拒绝定位时使用系统默认城市“成都”；摄像头预览、骨架画布不得遮挡操作说明；识别成功后自动关闭摄像头返回对话界面，流式输出中不触发关闭
