...
- 模型 API 快速配置页需回显当前用户已保存 API Key（默认密码框隐藏，可手动显示/隐藏）并支持保存前测试连接；API Base URL 必须由系统按 Provider 维护白名单，用户选择厂商后从该厂商白名单地址中下拉选择，保存与测试均需后端校验；变更 Provider/API Base/Model Name 且继续使用已保存密钥时，需显示明确的确认复用入口
...
- 用户侧手势识别需提供基本说明入口，明确展示 `剪刀手→@天气 [当前城市]`、`握拳→@随机音乐`、`手掌→@新闻聚合` 的触发关系、摄像头/定位权限要求和冷却规则；剪刀手查询天气时需使用浏览器系统定位，经后端反查城市后发送 `@天气 城市名`，摄像头启动后应后台预解析城市以降低首次触发等待，定位失败或用户拒绝定位时内部使用系统默认城市“成都”兜底但不在手势说明区展示该实现细节；摄像头预览、骨架画布不得遮挡操作说明；识别成功后自动关闭摄像头返回对话界面，流式输出中不触发关闭
