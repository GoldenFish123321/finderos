...
- 采集结果"标记保存"到仓库
- 管理侧数智大屏 `/admin/dashboard` 需以自适应比例完整呈现采集数据和系统运营态势：包含 3D 地球、词云、趋势图、来源分布、深采占比、来源 Top 和最新入库动态；3D 地球必须使用系统内置渲染能力（如原生 Canvas），不能要求用户额外安装浏览器插件或依赖 ECharts-GL 外部纹理，且必须使用真实来源聚合数据，按来源名称/域名推断或稳定兜底映射点位，标注来源名称、数量、今日新增、深采数量和域名，不能只显示单一内置来源或前端随机模拟点。
