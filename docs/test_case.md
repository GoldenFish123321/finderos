...
| TC-AUTH-07 | 缺少 OpenCV contrib 访问账户页 | 仅安装 opencv-python，登录后访问 /account | 页面返回 200，密码修改可用，人脸区域显示组件不可用 |
| TC-AUTH-08 | 缺少 OpenCV contrib 调用人脸登录 | 未登录状态提交人脸登录请求 | 返回受控错误，提示使用密码登录，不返回 500 |
