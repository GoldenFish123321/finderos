...
| TC-AUTH-08 | 人脸登录启用确认 | 未注册/刚注册人脸时查看“启用人脸识别登录” | 未注册时开关仍可交互并保存启用状态；若尝试人脸登录则提示尚未录入人脸；录入后用户可自由勾选启用/关闭 |
| TC-AUTH-09 | 人脸文件写入 | 项目位于中文路径时确认使用人脸照片 | 后端真实写入 `uploads/{username}.jpg`，保存失败时注册失败而不是假成功 |
| TC-AUTH-10 | 人脸登录反复测试 | 连续多次人脸登录失败/重试 | 不返回“操作过于频繁”，继续返回识别结果或具体配置/录入提示 |
| TC-AUTH-11 | 人脸识别稳定性 | 光照/裁剪略有变化的人脸登录 | 单模板识别使用最大脸候选、边距裁剪和 CLAHE 归一化，不要求重复录入多张照片 |
| TC-AUTH-12 | 缺少 OpenCV contrib 访问账户页 | 仅安装 opencv-python，登录后访问 `/account` | 页面返回 200，密码修改可用，人脸区域显示组件不可用 |
| TC-AUTH-13 | 缺少 OpenCV contrib 调用人脸登录 | 未登录状态提交人脸登录请求 | 返回受控错误，提示使用密码登录，不返回 500 |
| TC-MCP-01 | 全新数据库初始化脚本型采集工具 | 对空数据库依次执行 `init_db()`、`seed_default_data()` 两次 | 两次均成功，4 个采集工具各只有一条记录且保持 script 型 |
| TC-MCP-02 | 天气脚本工具真实调用 | 按应用启动顺序注册外部代理后调用 `weather_query(city=Beijing)` | 返回可渲染天气卡片的 wttr.in 嵌套对象，不返回未注册接口或转换错误 |
| TC-CHAT-GESTURE-01 | 手势说明入口 | `/chat` 点击顶部“说明”或打开手势区域 | 显示剪刀手/握拳/手掌对应的数字员工、摄像头/定位权限和冷却说明 |
| TC-CHAT-GESTURE-02 | 手势识别自动关闭 | 打开手势摄像头，做出剪刀手/握拳/手掌手势直至识别成功 | 消息自动发送，摄像头自动关闭并返回对话界面 |
| TC-CHAT-GESTURE-03 | 流式输出中手势不关闭 | 正在接收 AI 回复时做出识别手势 | 提示"请等待当前回复完成"，摄像头保持开启不关闭 |
| TC-CHAT-GESTURE-04 | 剪刀手天气定位 | 浏览器允许定位后做出剪刀手 | 系统经 `/api/location/reverse` 反查当前城市，并发送 `@天气 当前城市` |
| TC-CHAT-GESTURE-05 | 剪刀手天气默认城市 | 浏览器拒绝定位或定位失败后做出剪刀手 | 系统发送 `@天气 成都`，不发送空的 `@天气` |
| TC-CHAT-GESTURE-06 | 剪刀手天气首次触发延迟 | 打开手势摄像头后等待片刻再做出剪刀手 | 页面已后台预取当前城市，首次剪刀手不因才开始定位而长时间等待 |
| TC-CHAT-GESTURE-07 | 手势说明文案简洁 | 查看手势区域和说明弹窗 | 不展示“定位失败默认成都”等内部兜底文案 |
| TC-WATCH-SOURCE-01 | 三个默认瞭望源 | 初始化全新数据库和仅有百度源的旧数据库 | 均存在百度新闻、搜狗搜索、Bing RSS 三个启用源，重复初始化不产生重复数据 |
| TC-WATCH-SOURCE-02 | 三源真实采集与落库 | 使用同一关键词分别执行三个默认源采集 | 每个源返回成功状态和解析结果，`watch_results` 可按对应 `source_id` 查询到记录 |
| TC-WATCH-SOURCE-03 | 解析器配置联动 | 分别从管理页面、MCP 工具和定时任务触发采集 | 三条链路均使用瞭望源配置的 parser，不再根据 URL 猜测 |
## 2. 用户管理
...
| TC-USER-07 | 统一密码策略 | 分别在管理员新增/重置、后台自助修改和账户设置提交6位密码，再提交8位两类字符密码 | 所有入口拒绝弱密码并接受合规密码，提示内容一致 |
...
| TC-MODEL-08 | 快速配置测试连接 | 从 Provider 白名单下拉选择 API Base，填写 API Key/Model Name 后点击测试连接 | 返回 HTTP 状态、耗时与成功/失败提示，不展示或记录密钥；非白名单 API Base 被后端拒绝 |
| TC-MODEL-09 | 变更接口地址防误发 Key | 修改 Provider 或 API Base 后继续使用已保存 Key | 页面显示“复用当前密钥”确认区；未勾选时拒绝保存/测试，勾选后允许复用；API Base 只能从所选厂商白名单中选择 |
...
| TC-CHAT-08 | 对话历史管理 | C 区创建/切换/删除对话 | 对话列表正确更新；删除失败需提示原因 |
| TC-CHAT-08b | 删除会话防复活 | 删除当前会话后，模拟迟到的流式回复尝试保存消息 | 已删除会话不重新出现在列表，消息不被写入 |
...
| TC-REG-04 | 弱密码 | 输入少于8位或仅含一种字符类型的密码 | 提示至少8位且必须包含至少两种字符类型 |
...
| TC-DASH-01 | 数智大屏关键词排行 | 管理员访问 `/admin/dashboard`，存在热门关键词数据 | 页面正常渲染 Top 10 排行，不因模板 `loop.index` 变量报 500 |
| TC-DASH-02 | 数智大屏来源地球 | 数据仓库包含百度、GitHub、自定义接口等多个来源 | 3D 地球在数智大屏模块内优先以 Three.js/WebGL 数字地球渲染真实地球纹理、云层、大气辉光、飞线和来源点位；不要求用户安装浏览器插件，不依赖 ECharts-GL；CDN/WebGL 不可用时自动回退 Canvas 地球；按真实来源聚合显示点位，标注来源名称、数量、今日新增、深采数量和有效域名，`example`/`example.*` 演示域名不展示；未知来源也有稳定兜底点位 |
| TC-DASH-03 | 数智大屏自适应布局 | 不同浏览器宽度访问 `/admin/dashboard` | 页面不被后台头部/侧栏遮挡，整体采用主地球、右侧信息宫格和底部等高图表排布；统计卡片、3D 地球、词云、关键词排行、来源列表、最新动态、趋势图和占比图完整显示 |
