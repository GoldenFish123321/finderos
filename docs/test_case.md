...
| TC-DASH-02 | 数智大屏来源地球 | 数据仓库包含百度、GitHub、自定义接口等多个来源 | 3D 地球在数智大屏模块内优先以 Three.js/WebGL 数字地球渲染真实地球纹理、云层、大气辉光、飞线和来源点位；不要求用户安装浏览器插件，不依赖 ECharts-GL；CDN/WebGL 不可用时自动回退 Canvas 地球；按真实来源聚合显示点位，标注来源名称、数量、今日新增、深采数量和域名；未知来源也有稳定兜底点位 |
