...
| TC-DASH-02 | 数智大屏来源地球 | 数据仓库包含百度、GitHub、自定义接口等多个来源 | 系统内置 3D 地球渲染，不要求用户安装浏览器插件或依赖 ECharts-GL 外部纹理；按真实来源聚合显示点位，标注来源名称、数量、今日新增、深采数量和域名；未知来源也有稳定兜底点位 |
| TC-DASH-03 | 数智大屏自适应布局 | 不同浏览器宽度访问 `/admin/dashboard` | 页面不被后台头部/侧栏遮挡，统计卡片、3D 地球、词云、趋势图、来源列表和最新动态完整显示 |
