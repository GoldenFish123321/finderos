...
```
         ...
         │      MCP 工具（对 LLM 暴露的能力）          │
         │   = 本地接口数据源 × N + 纯数据转换脚本     │
         │   脚本接收本地接口数据，返回字符串（可以是纯文本或 JSON 字符串）│
         └──────────────┬───────────────────────────┘
                        │ 引用（仅 local）
         ┌──────────────▼───────────────────────────┐
         │      接口管理（统一代理所有出口流量）        │
...
**一句话总结**：接口管理是系统除LLM的API调用外的**唯一的对外网络出口**。所有外部 API 在接口管理层注册后自动包装为本地接口（虚拟 local handler），对内透明代理。脚本层只能引用本地接口，完全感知不到外网——脚本做纯数据转换，返回字符串作为 MCP 工具结果（前端按内容类型自动渲染）。
...
│  脚本只能引用本地接口，返回字符串（可以是纯文本或 JSON 字符串，直接作为 MCP text content）      │
├──────────────────────────────────────────────────────────────────┤
...
│  · 输出: str（纯文本或 JSON 字符串）— 直接作为 MCP tools/call 的 text content            │
...
| **脚本极简化** | 脚本做纯数据→字符串转换（含JSON格式化），禁 import、禁网络、禁文件。AST 沙箱从 ~200 行简化到 ~130 行 |
...
| **MCP 协议原生对齐** | 脚本返回 `str`（纯文本或 JSON 字符串），直接映射为 MCP `text content`，LLM 友好，无需额外 JSON 包装 |
