...
- 🧠 **MCP 协议**：18+ 数据库驱动的标准化工具，LLM Function Calling 智能决策调用
- 💬 **智能问数**：用户前台 SSE 流式对话
- 🤖 **数字员工**：LLM 型 / API 型双模式，@ 触发自动匹配，MCP 工具权限控制
- 🎯 **技能管理**：Prompt 增强 + MCP 工具关联，`load_skill` 按需加载
- 🔧 **MCP 工具管理**：数据库驱动的工具注册中心，支持热重载/在线测试/分类管理 (v0.6.0)
